...
export function VideoCarouselBasicExample() {
    ...
    return (
        <div className="relative w-full md:h-[760px] top-0 z-0 overflow-hidden">
            ...
            {/* Video Clip Previews Navigation */}
            <div className="absolute bottom-4 left-0 right-0 flex justify-center space-x-2 md:space-x-4 px-4">
                {slides.map((slide, index) => (
                    <div 
                        key={index} 
                        className={`relative cursor-pointer transition-all duration-300 ${index === current ? 'ring-2 ring-white scale-110' : 'opacity-80 hover:opacity-100'}`}
                    ...
                    >
                        {isMobile ? (
                            ...
                            <video 
                                ...
                                ref={el => {
                                    if (el) {
                                        el.currentTime = 1; // Start at 1 second to show content
                                        el.play().catch(e => console.log("Preview autoplay prevented"));
                                    }
                                }}
                            >
                                <source src={slide.src} type="video/mp4" />
                        ...
                        {index === current && (
                            <div className="absolute bottom-0 left-0 right-0 h-1 bg-white animate-progress" 
            ...
            {isMobile ? (
                <div className="absolute inset-x-[5%] md:inset-x-[10%] top-[40%] md:top-[50%] transform -translate-y-1/2 py-4 text-center text-white">
                    <br /> <br /> <br />
                    <b><h5 className="text-2xl md:text-xl">Content Head</h5></b>
                    <p className="text-[16px] md:text-base w-[100%] font-afacad p-[20px] md:w-2xl">
                        Lorem ipsum dolor sit amet consectetur adipisicing elit. Voluptatibus, tempora voluptatem! Recusandae, vero debitis! Quis illum cupiditate! Tempore sunt dolorum quis.
                    </p>
                    <Button />
                </div>
            ) : (
