...
function Counter() {
  ...
  return (
    <section 
      ref={sectionRef}
      className="flex items-center sm:pb-[50px] lg:h-[100%] justify-center px-6 sm:px-2.5 bg-gradient-to-b from-[#ffffff] via-[#ffffff] to-[#e8e8e849]"
    >
      <div className="mx-auto text-center">
        <br />
        <br />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 pb-[30px] md:grid-cols-3 gap-10">
          {CounterData.map((item, index) => (
            <div 
              key={index} 
              className="px-10 py-8 rounded-lg bg-[#5e6c84] shadow-lg transform hover:scale-105 transition duration-300" 
              data-aos="fade-up"
              data-aos-delay={index * 150} // Staggered animations
            >
              <h3 className="text-6xl font-bold text-[#3795c4] mb-2">
                {counts[index]}+
              </h3>
              <p className="text-3xl text-white font-semibold">{item.label}</p>
              <p className="text-gray-300 text-xl mt-2">{item.desc}</p>
            </div>
          ))}
        </div>
      </div>
